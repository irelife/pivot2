...
function renderStats(){
  ...
  const stats = [
    {lbl:'登録',     num:total,    cls:''},
    {lbl:'不備あり', num:defect,   cls:'warn'},
    {lbl:'進行中',   num:progress, cls:'accent'},
    {lbl:'もうすぐ', num:almost,   cls:'almost'},
    {lbl:'完了',     num:complete, cls:'success'}
  ];
  document.getElementById('stats-bar').innerHTML = stats.map(s =>
    '<div class="stat-pill ' + s.cls + (s.lbl==='完了' ? ' clickable" onclick="KB.openDoneModal()"' : '"') + '><span class="num">' + s.num + '</span><span class="lbl">' + s.lbl + (s.lbl==='完了' ? ' ▸' : '') + '</span></div>'
  ).join('');
  drawArchiveNotice();
  syncViewBtn();
}
...
function renderListView(list){
  const row = c => {
    ...
    return '<tr class="lv-row ' + cls + '" onclick="KB.onCardClick(event,\'' + c.id + '\')">'
      ...
      + '<td>' + esc(c.broker || '') + '</td>'
      + '<td>' + esc(c.staff || '') + '</td>'
      + '<td class="lv-c">' + (c.contractDate ? esc(c.contractDate.replace(/-/g,'/')) : '—') + '</td>'
  ...
  return '<div class="lv-wrap"><table class="lv-table">'
    ...
    + '<th>仲介業者</th><th>担当</th><th>契約日</th><th>進捗</th></tr></thead>'
...
/* BROKER_HISTORY.length = 0;  ← 検証用に履歴を捨てる行。集計を戻すため無効化 */
// 履歴の客付時期を取り出す（統計互換）
